...
def main():
    ...
    # Retrieve data from the 'forwarded' collection
    forwarded_data = list(db['forwarded'].find({}, {'_id': 0}))
    ...
    if forwarded_data:
        ...
        for index, row in df.iterrows():
            ...
            for col in df.columns:
                st.text_input(col, row[col], disabled=True)
            ...
            if st.button("Write Review", key=f"write_review_{index}"):
